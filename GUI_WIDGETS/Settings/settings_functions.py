...
class Settings_funcs:
	...
	def wifiSetup(self):
		...
		def connectWifi():
			print('Attempting to connect to "', self.GUI_Settings.bearing_wifiSSIDInput.text().strip(), '" with key "', self.GUI_Settings.bearing_wifiPassInput.text().strip(), '"')
			if subprocess.run(f'iwconfig wlan0 essid {self.GUI_Settings.bearing_wifiSSIDInput.text().strip()} key {self.GUI_Settings.bearing_wifiPassInput.text().strip()}', capture_output=True, shell=True).returncode == 0:
				refresh()
			else:
