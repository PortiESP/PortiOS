	...
	def mediaPlayerThreadFunc(self):
		...
		while 1:
			...
			if self.isConnectedDevice == False and checkDevice == True:
				...
				self.GUI_Central.bluetoothStatusButton.setIcon(icon)
				self.GUI_Central.slider_volume.setValue(self.volumeIface.Get('org.bluez.MediaTransport1', 'Volume'))
				if str(self.volumeIface.Get('org.bluez.MediaPlayer1', 'Status')) == 'playing':
